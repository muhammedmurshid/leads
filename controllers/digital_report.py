...
class LeadExcelDigitalReportController(http.Controller):
    @http.route([
        '/digital_lead_report/excel_report/<model("lead.report"):report_id>',
    ], type='http', auth="user", csrf=False)
    def get_sale_excel_digital_report(self, report_id=None, **args):
        print(report_id, 'report_id')
        datas = report_id.datas_ids
        print(datas, 'date')
        lead_source = request.env['leads.sources'].sudo().search([('digital_lead', '=', True)])
        total_hot = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '=', 'hot'),('leads_source', 'in', lead_source.ids)])
        total_cold = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '=', 'cold'),('leads_source', 'in', lead_source.ids)])
        total_warm = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '=', 'warm'),('leads_source', 'in', lead_source.ids)])
        total_nil = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '=', 'nil'),('leads_source', 'in', lead_source.ids)])
        total_bad = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '=', 'bad_lead'),('leads_source', 'in', lead_source.ids)])
        total_leads_count = request.env['leads.logic'].sudo().search_count(
            [('id', 'in', datas.ids), ('lead_quality', '!=', False),
             ('leads_source', 'in', lead_source.ids)])
        # total_leads = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '!=', False),('leads_source', 'in', lead_source.ids)])
        total_adm = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids),('admission_status', '=', True),('leads_source', 'in', lead_source.ids)])
        adm_hot = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids),('admission_status', '=', True),('lead_quality', '=', 'hot'),('leads_source', 'in', lead_source.ids)])
        adm_cold = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids),('admission_status', '=', True),('lead_quality', '=', 'cold'),('leads_source', 'in', lead_source.ids)])
        adm_warm = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids),('admission_status', '=', True),('lead_quality', '=', 'warm'),('leads_source', 'in', lead_source.ids)])
        adm_nil = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids),('admission_status', '=', True),('lead_quality', '=', 'nil'),('leads_source', 'in', lead_source.ids)])
        adm_bad = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids),('admission_status', '=', True),('lead_quality', '=', 'bad_lead'),('leads_source', 'in', lead_source.ids)])

        google_hot = request.env['leads.logic'].sudo().search_count(
            [('id', 'in', datas.ids), ('lead_quality', '=', 'hot'),
             ('incoming_source', '=', 'google')])
        google_warm = request.env['leads.logic'].sudo().search_count(
            [('id', 'in', datas.ids), ('lead_quality', '=', 'warm'),
             ('incoming_source', '=', 'google')])
        google_cold = request.env['leads.logic'].sudo().search_count(
            [('id', 'in', datas.ids), ('lead_quality', '=', 'cold'),
             ('incoming_source', '=', 'google')])
        google_nil = request.env['leads.logic'].sudo().search_count(
            [('id', 'in', datas.ids), ('lead_quality', '=', 'nil'),
             ('incoming_source', '=', 'google')])
        google_bad = request.env['leads.logic'].sudo().search_count(
            [('id', 'in', datas.ids), ('lead_quality', '=', 'bad_lead'),
             ('incoming_source', '=', 'google')])
        ...
        report_lines = report_id.get_digital_report_lines()
