...
class LeadExcelDigitalReportController(http.Controller):
    ...
    def get_sale_excel_digital_report(self, report_id=None, **args):
        ...
        total_hot = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '=', 'hot')])
        total_cold = request.env['leads.logic'].sudo().search_count([('id', 'in', datas.ids), ('lead_quality', '=', 'cold')])
