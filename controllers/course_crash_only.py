...
class LeadExcelReportCourseCrashController(http.Controller):
    @http.route([
        '/lead_report_crash_course/excel_report/<model("lead.report"):report_id>',
    ], type='http', auth="user", csrf=False)
    def get_course_crash_base_excel_report(self, report_id=None, **args):
        ...
        headers = []
        report_lines = report_id.get_crash_course_reports()
            ...
            for source in lead_source:
                ...
                search_count = request.env['leads.logic'].sudo().search_count(
                    [('id', 'in', datas.ids), ('leads_source', '=', source.id),
                     ('base_course_id', '=', line['course_id']), ('course_type', '=', 'crash')])
        ...
        for adm in lead_source:
            ...
            adm_count = request.env['leads.logic'].sudo().search_count(
                [('id', 'in', datas.ids), ('leads_source', '=', adm.id), ('admission_status', '=', True), ('course_type', '=', 'crash')
                 ])
        ...
        for e in lead_source:
            ...
            search_prc = request.env['leads.logic'].sudo().search_count(
                [('id', 'in', datas.ids), ('leads_source', '=', e.id), ('course_type', '!=', 'crash')])
